...
psql -h db -U postgres -d fafusers -c "DELETE FROM megausers WHERE comrades = '$extr_tbl' AND date = '$cur_date';"
api_id_encoded=$(echo -n "$api_id" | jq -sRr @uri)

curl --data "chat_id=$chat_id&text=$extract" https://api.telegram.org/bot$api_id_encoded/sendMessage? && sleep 1 ;

bash ./random.sh > ./config/result.md
extract_2=$(cat ./config/result.md)

curl --data "chat_id=$chat_id&text=$extr_tbl : $extract_2" https://api.telegram.org/bot$api_id_encoded/sendMessage?
